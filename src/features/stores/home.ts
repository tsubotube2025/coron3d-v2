...
const homeStore = create<HomeState>()(
  persist(
    (set, get) => ({
      ...
      backgroundImageUrl:
        process.env.NEXT_PUBLIC_BACKGROUND_IMAGE_PATH ?? 'green',
      modalImage: '',
    ...
    }),
    {
      ...
      partialize: ({ chatLog, showIntroduction, backgroundImageUrl }) => ({
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          ...
          // 既存のユーザーが古いデフォルト値を使用している場合、新しいデフォルト値にマイグレーション
          if (state.backgroundImageUrl === '/backgrounds/bg-c.png') {
            state.backgroundImageUrl = 'green'
            console.log('Migrated backgroundImageUrl to green')
          }
        }
      },
    }
